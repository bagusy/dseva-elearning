...
class StoreVideoRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('create video');
    }
    ...
    protected function prepareForValidation()
    {
        ...
        if (str_contains($this['link'], 'wistia.com/medias/')) {
            $parts = explode('wistia.com/medias/', $this['link'], 2);
            if (count($parts) < 2 || empty($parts[1])) {
                throw ValidationException::withMessages([
                    'link' => ['This link is not a valid wistia link'],
                ]);
            }
            $this['link'] = 'https://fast.wistia.net/embed/iframe/' . $parts[1];
